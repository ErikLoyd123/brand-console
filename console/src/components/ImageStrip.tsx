...
// Which producer Image with AI uses.
//
// 'auto' is the DEFAULT and the right answer nearly always: nothing is pinned, and the
// session picks the best model for each image's TYPE from src/images/recommend.ts —
// which reconciles "what's the right model for this job?" against "what is actually
// installed on this machine?". Because diffusion can't spell and Claude can't
// photograph, the type decides the model far more reliably than a pick made before
// anyone knows what the image will be.
//
// The other two kinds are the OVERRIDE — they force a producer and make the session skip
// the type menu: 'local' = generated by the named on-machine model entry (from
// image-generation.config.json), 'claude' = a composed figure Claude authors in the
// brand's language, optionally pinning which Claude model runs the session.
...
export function ImageStrip({
  ...
  // Model picker. The default is "auto" — the session picks the best model per image
  // type against what's installed (src/images/recommend.ts). Everything below it is an
          ...
          {onImageAI && (
            <span className="inline-flex items-center gap-1">
              ...
              <span className="group relative inline-flex">
                ...
                >
                  <>
                    <span className="font-medium text-text">Best for the job</span> is the
                    default and usually right: nothing is pinned, so the session picks a model
                    per image once it knows the <em>type</em> — and only from what&rsquo;s
                    installed here. Diffusion can&rsquo;t spell and Claude can&rsquo;t
                    photograph, so the type decides the model better than a pick made before
                    anyone knows what the image is. It tells you what it chose and why.{' '}
                    {usableModels.length === 0 && generator !== null && (
                      <>
                        No local image model is set up, so anything photoreal is off the menu
                        until one is.{' '}
                      </>
                    )}
                    The <span className="font-medium text-text">Force</span> options override
                    that: they pin one producer for every image in the run and skip the type
                    menu.{' '}
                    <a
                      href="#/imagemodels"
                      className="font-medium text-primary-ink underline-offset-2 hover:underline"
                    >
                      See how each model does →
                    </a>{' '}
                    ·{' '}
                    <a
                      href="#/connections"
